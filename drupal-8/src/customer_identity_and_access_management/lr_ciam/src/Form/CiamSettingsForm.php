...
class CiamSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $thanks_text = $this->t('To activate the module, you will need to first configure it (manage your desired social networks, etc.) from your LoginRadius account. If you do not have an account, click <a href="@loginradius" target="_blank">here</a>.<br/>We also offer Social Plugins for
  ...
  <a href="@dotnetnuke" target="_blank">DotNetNuke</a>  <br/><a href="@loginradius" target="_blank"><br/><input class="form-submit" type="button" value="Set up my account!"></a> (<a href="@get_sociallogin" target="_blank">How to set up an account?</a>)', [
    ...
    if ($config->get('api_key') != '' && $config->get('api_secret')) {
      ...
      if (isset($is_phone_login) &&  $is_phone_login) {
        $form['phone_warning_block'] = [
          ...
          '#description' => $this->t('If you login with phone number then a random mail id will be generated on the basis of phone@yourdomain.com'),
        ];
      }
    }
    ...
    $form['lr_basic_settings']['login_redirection'] = [
      '#type' => 'radios',
      '#title' => $this->t('Redirection settings after login'),
